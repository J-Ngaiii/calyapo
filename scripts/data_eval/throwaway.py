...
def generate_best_plot(df, split, score: str):
    ...
    plt.title(f'Best Model per Demographic Group (Min Weighted {score} on Test Set)', fontsize=14)
...
if __name__ == "__main__":
    cols = ['KL_Unweighted', 'WD_Unweighted', 'KL_Weighted', 'WD_Weighted']
    df = pd.read_csv('inference_outputs/opinion_school/reports_aurora/results/distributional_accuracy/summary_demog_metrics.csv')
    generate_best_plot(df=df, split='test', score='KL_Weighted')