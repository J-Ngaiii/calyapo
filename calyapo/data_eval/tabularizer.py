...
class Tabularizer:
    ...
    def run_pipeline(self, model_map: Dict[str, str]):
        ...
        for split in partitions:
            d_path = Path('calyapo/data') / Path(f'final_{self.keyword}') / f"{self.train_plan}_{split}.jsonl"
            m_path = Path('calyapo/data') / Path(f'final_{self.keyword}') / f"{self.train_plan}_{split}_meta.jsonl"
