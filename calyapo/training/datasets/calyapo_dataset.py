...
class CalyapoDataset(Dataset):
    def __init__(self, dataset_config: calyapo_dataset_config, tokenizer, filepath):
        """
        Args:
            ...
            partition: filepath of train or val or test file
                - we let get_preprocessed_dataset()'s internal get_split() function pull the path
                - this function is loacted in training/utils/dataset_utils.py
                - it gets called in finetuning.py to construct the datasets
        """
    ...
    def update_eos_pred(self, tf: bool):
        self.predict_eos = tf
        print(f"(calyapo_dataset obj | Debug) updated EOS awarness: {'ENABLED' if self.predict_eos else 'DISABLED'}")
...
def get_calyapo_dataset(dataset_config, tokenizer, filepath):
    """
    Entry point function used by the Llama Cookbook loader.
    """
    return CalyapoDataset(dataset_config, tokenizer, filepath)
